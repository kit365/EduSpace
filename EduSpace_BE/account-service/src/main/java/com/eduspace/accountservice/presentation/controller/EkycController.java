...
@RestController
@RequestMapping(AccountPaths.BASE_PATH + AccountPaths.ME + "/ekyc")
...
public class EkycController {
    ...
    public ApiResponse<EkycVerifyResponse> verify(
        ...
        EkycVerifyResponse data = ekycVerificationService.verify(keycloakId, email, front, back, selfie);
    ...
    public ApiResponse<EkycVerifyResponse> commit(
        ...
        String keycloakId = jwt.getSubject();
        String email = jwt.getClaimAsString("email");
        EkycVerifyResponse data = ekycVerificationService.commitFromClient(keycloakId, email, request);
        String message = messageSource.getMessage(
                SuccessCode.EKYC_VERIFY_SUCCESS.getMessageKey(),
                null,
                SuccessCode.EKYC_VERIFY_SUCCESS.getMessageKey(),
